...
class rob_d2v_cc(nn.Module):            
    def __init__(self, config):        
        super().__init__()
        self.roberta_model = RobertaModel.from_pretrained('/data01/xdy/MMML/roberta-large')
        self.data2vec_model = Data2VecAudioModel.from_pretrained("/data01/xdy/MMML/data2vec-audio-large-960h/")
...
class rob_d2v_cme(nn.Module):            
    def __init__(self, config):        
       ...
       # 动态获取隐藏层大小
        roberta_hidden_size = self.roberta_model.config.hidden_size  # 1024 for roberta-large
        data2vec_hidden_size = self.data2vec_model.config.hidden_size  # 具体数值根据模型不同

     # 你需要加上 fused_output_layers，输入维度是两者相加：
        self.fused_output_layers = nn.Sequential(
        nn.Dropout(config.dropout),
        nn.Linear( roberta_hidden_size + data2vec_hidden_size, 768),  # 这里改成拼接后的维度
        nn.ReLU(),
        nn.Linear(768, 768),
        nn.ReLU(),
        nn.Linear(768, 512),
        ...
        self.T_output_layers = nn.Sequential(
            nn.Dropout(config.dropout),
            nn.Linear(roberta_hidden_size, 1)
        )
        self.A_output_layers = nn.Sequential(
            nn.Dropout(config.dropout),
            nn.Linear(data2vec_hidden_size, 1)
        )

        # 修改Embedding层的embedding_dim为动态值，替代硬编码768
        self.text_cls_emb = nn.Embedding(num_embeddings=1, embedding_dim=roberta_hidden_size)
        self.audio_cls_emb = nn.Embedding(num_embeddings=1, embedding_dim=data2vec_hidden_size)
        if self.version == 'v3':
            self.text_mixed_cls_emb = nn.Embedding(num_embeddings=1, embedding_dim=roberta_hidden_size * 2)
            self.audio_mixed_cls_emb = nn.Embedding(num_embeddings=1, embedding_dim=data2vec_hidden_size * 2)
        else:
            self.text_mixed_cls_emb = nn.Embedding(num_embeddings=1, embedding_dim=roberta_hidden_size)
            self.audio_mixed_cls_emb = nn.Embedding(num_embeddings=1, embedding_dim=data2vec_hidden_size)
       ...
       # self.T_output_layers = nn.Sequential(
        ...
        # fusion method V3
        if self.version == 'v3':
            encoder_layer = nn.TransformerEncoderLayer(d_model=roberta_hidden_size*2, nhead=16, batch_first=True)
            ...
            encoder_layer = nn.TransformerEncoderLayer(d_model=roberta_hidden_size, nhead=16, batch_first=True)
            self.text_encoder = nn.TransformerEncoder(encoder_layer, num_layers=3,enable_nested_tensor=False)
            encoder_layer = nn.TransformerEncoderLayer(d_model=roberta_hidden_size, nhead=16, batch_first=True)
            self.audio_encoder = nn.TransformerEncoder(encoder_layer, num_layers=3,enable_nested_tensor=False)
        ...
        if self.version == 'v3':
            self.fused_output_layers = nn.Sequential(
                nn.Dropout(config.dropout),
                nn.Linear(roberta_hidden_size*4, 768),
                nn.ReLU(),
                nn.Linear(768, 512),
                nn.ReLU(),
                nn.Linear(512, 1)
            ...
            self.fused_output_layers = nn.Sequential(
                nn.Dropout(config.dropout),
                nn.Linear(roberta_hidden_size*2, 768),
                nn.ReLU(),
                nn.Linear(768, 512),
                nn.ReLU(),
                nn.Linear(512, 1)
    ...
    def forward(self, text_inputs, text_mask, audio_inputs, audio_mask, text_context_inputs=None, audio_context_inputs=None):
    # text feature extraction
        raw_output = self.roberta_model(text_inputs, text_mask)
    ...
    # CME layers
        if text_context_inputs is not None and audio_context_inputs is not None:
        # Prepend context to the input if context is provided
        ...
        # If no context, just use the main inputs
            text_inputs, text_attn_mask = self.prepend_cls(T_hidden_states, text_mask, 'text')
            audio_inputs, audio_attn_mask = self.prepend_cls(A_hidden_states, audio_mask_new, 'audio')

    # pass through CME layers
        for layer_module in self.CME_layers:
            text_inputs, audio_inputs = layer_module(text_inputs, text_attn_mask, audio_inputs, audio_attn_mask)
